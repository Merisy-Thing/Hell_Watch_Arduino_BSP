...
void Gamebuino::changeGame(){
	...
	#define bootloader_vect (*((void(*)(const char* filename))(0x4000)))
	bootloader_vect("LOADER");
#else
	display.clear();
...
#ifdef HELL_WATCH
uint8_t Gamebuino::rom_read_byte(uint32_t addr) {
	uint8_t buf=0xFF;
...
uint16_t Gamebuino::rom_read_word(uint32_t addr) {
	uint8_t buf[2];

	buf[0] = rom_read_byte(addr);
	buf[1] = rom_read_byte(addr+1);

	return ((uint16_t)buf[1] << 8) | buf[0];
}

boolean Gamebuino::settingsAvailable(){
	if(rom_read_word(SETTINGS_PAGE) == SETTINGS_TOKEN)
		return 1;
	else
		return 0;
}
...
void Gamebuino::readSettings(){
	if(settingsAvailable()){
		display.contrast = rom_read_byte(SETTINGS_PAGE+OFFSET_CONTRAST);
		backlight.backlightMin = rom_read_byte(SETTINGS_PAGE+OFFSET_BACKLIGHT_MIN);
		backlight.backlightMax = rom_read_byte(SETTINGS_PAGE+OFFSET_BACKLIGHT_MAX);
		backlight.ambientLightMin = rom_read_word(SETTINGS_PAGE+OFFSET_LIGHT_MIN);
		backlight.ambientLightMax = rom_read_word(SETTINGS_PAGE+OFFSET_LIGHT_MAX);
		
		sound.volumeMax = rom_read_byte(SETTINGS_PAGE+OFFSET_VOLUME_MAX);
		sound.globalVolume = rom_read_byte(SETTINGS_PAGE+OFFSET_VOLUME_DEFAULT);
...
void Gamebuino::getDefaultName(char* string){
	if(settingsAvailable()){	
		for(byte i=0; i<USERNAME_LENGTH; i++){
			string[i] = (char)rom_read_byte(SETTINGS_PAGE+OFFSET_USERNAME+i);
		}
	}
}
#else
